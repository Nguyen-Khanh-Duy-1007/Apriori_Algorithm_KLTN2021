...
public class AprioriImproved extends Observable {
    public static void main(String[] args) throws Exception {
        AprioriImproved ap = new AprioriImproved(args);
    }
    private int [][] aList=new int[1000][100001];
    ...
    File file = new File("/home/khanhduyuser/AprioriJava/outputAprioriImprove/output.txt");
    private boolean del = file.delete();

    /** This is the main interface to use this class as a library */
    public  AprioriImproved(String[] args, Observer ob) throws Exception
    {
    	usedAsLibrary = true;
    	configure(args);
    	this.addObserver(ob);
     ...
     */
    public  AprioriImproved(String[] args) throws Exception
    {
        configure(args);
        go();
    }
    ...
    private void foundFrequentItemSet(int[] itemset, int support) {
    	...
    	try {
		File fout = new File("/home/khanhduyuser/AprioriJava/outputAprioriImprove/output.txt");
   ...
   /** outputs the current configuration
	...
	int index=itemsets.get(0).length;
	int dem=0;
	if (index==1)
	{
		for (int c = 0; c < itemsets.size(); c++) {
			int[] cand = itemsets.get(c);
			int min=aList[cand[0]][0];
			int ind=cand[0];
			for (int temp=1; temp<cand.length;temp++)
			{
				if (aList[cand[temp]][0]<min)
				{
					min=aList[cand[temp]][0];
					ind=cand[temp];
				}
			}
